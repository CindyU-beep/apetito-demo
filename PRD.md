...
### AI Semantic Search in Browse Products
- **Functionality**: Intelligent search capability in the Browse Products page powered by Azure-inspired semantic search using the Spark LLM API. Toggle-able AI search mode that understands natural language queries and search intent beyond simple keyword matching, analyzing meal characteristics, dietary preferences, nutritional goals, cuisine types, and contextual meaning to surface the most relevant meals from the catalog.
- **Purpose**: Enable institutional buyers to find meals using natural, conversational queries like "healthy protein-rich meals" or "vegetarian comfort food" rather than requiring exact keyword matches. Dramatically improve search relevance and discovery by understanding semantic meaning, allowing users to search by characteristics, dietary goals, cooking styles, and meal occasions rather than memorizing exact meal names.
- **Trigger**: User navigates to "Browse Products" tab → Clicks "AI Search" toggle button in search controls → Types natural language query in search input field → System debounces input (800ms) and automatically triggers semantic search when query length exceeds 3 characters
- **Progression**: User clicks AI Search toggle button (transitions from "Off" to "On" state with Sparkle icon filling) → Search placeholder updates to show example natural language queries → Informational banner appears explaining AI search capabilities → User types contextual query (e.g., "low carb dinner options", "meals with chicken and vegetables", "comfort food for cold weather") → System shows "Analyzing your search with AI..." loading indicator after 800ms debounce → LLM analyzes query against full meal catalog considering semantic meaning, dietary attributes, nutritional profiles, ingredients, meal types, and contextual intent → AI returns ranked meal IDs with explanation and confidence score → Results display with "AI-ranked by relevance" indicator → AI Analysis card shows explanation of matching logic and confidence percentage → Category filters continue to work, further refining AI-suggested results → Results update instantly when category changes without re-running AI search → User can toggle AI search off to return to traditional keyword matching → Clear button (X icon) resets search and clears AI results
- **Success criteria**: AI search toggle clearly indicates on/off state with visual feedback; Placeholder text adapts based on search mode; Debouncing prevents excessive API calls (800ms delay); Loading indicator appears during AI processing; Search completes within 2-3 seconds; AI explanation card provides clear reasoning for match logic; Confidence scores display as percentages; Semantic search understands dietary terms (vegetarian, vegan, meat-free, gluten-free); Cuisine and style queries work (Italian, comfort food, light meals); Nutritional goal queries return relevant results (high protein, low carb, healthy); Ingredient-based queries match meals containing those components; Category filters work seamlessly with AI results; Toggle off returns to standard keyword search without errors; Empty state messaging adapts based on search mode (suggests rephrasing for AI, suggests filter changes for keyword); Results maintain visual consistency with standard search; AI search state persists during session; Clear button removes search text and AI results simultaneously
...
- **Functionality**: Weekly meal planning interface with horizontal day layout showing meal cards, **meal catalog featuring traditional English meals** (Fish & Chips, Shepherd's Pie, Full English Breakfast, Roast Dinners, Sticky Toffee Pudding, etc.) from Apetito's British menu, comprehensive AI-powered capabilities including auto-generation, custom natural language requests, and deep nutritional analysis, smart recommendations based on institutional requirements, dietary restrictions, budget constraints, and person count, automatic shopping list generation with cost estimation, and **one-click "Place Order" button to add all planned meals to cart**. **AI Assistant automatically loads and references organization profile context** to provide personalized recommendations without requiring repetitive specification of dietary needs, allergen exclusions, budget targets, or serving capacity.
- **Purpose**: Enable institutional buyers to plan weekly menus in advance by selecting complete **English meals** from Apetito's catalog, leverage advanced AI to optimize nutritional balance and variety based on specific institutional needs **with automatic integration of organization profile data**, receive intelligent meal suggestions tailored to custom requirements through natural language **that respect saved allergen exclusions and dietary preferences**, visualize nutritional totals across days, track estimated costs per person **against organization budget targets**, and **seamlessly convert approved meal plans into cart orders with a single click**
- **Trigger**: User navigates to "Plan" tab; clicks "Create New Meal Plan" or selects existing plan; browses **English meals** in "Meals" tab; accesses "AI Assistant" tab with three modes (Generate, Custom, Analyze); **profile data auto-loads from saved organization settings**; configures additional institutional parameters (people count defaults to profile serving capacity, dietary restrictions pre-populate from profile, budget defaults to profile per-serving target); types natural language requests for plan modifications; clicks "Auto-Fill Week" for AI-generated complete weekly plans **with English meals**; requests analysis of current plan balance; **clicks "Place Order" button in Shopping List tab to add all meals to cart**
- **Progression**: User creates weekly plan → Accesses AI Assistant with tabbed interface → **Profile context banner displays** showing active organization (name, type, capacity, budget target, allergen exclusions highlighted in red, dietary preferences highlighted in green, special requirements) → **Generate Mode**: Views pre-filled institutional parameters from profile (number of people defaults to serving capacity, dietary restrictions auto-populate from profile preferences, budget per meal shows profile target) → Can override defaults if needed → Clicks "Auto-Fill Entire Week" → **AI generates complete balanced plan featuring English meals using profile context** (excludes all meals containing profile allergen exclusions, prioritizes meals matching profile dietary preferences, considers profile budget per serving, scales to profile serving capacity, **selects from traditional British menu items**) → Plan shows reasoning explicitly mentioning profile alignment ("Selected for vegetarian preference at St. Mary's Hospital", "Within $3.75/serving budget target") → **Custom Mode**: Types natural language requests ("Make Monday lighter", "Add more protein", "Swap meals with nuts", "Make Friday special") → **AI interprets request with full awareness of organization profile and English meal catalog** (automatically excludes allergens, respects dietary preferences, considers budget constraints, references order history, **suggests appropriate English meal alternatives**) → Provides summary of changes made with profile compliance noted → **Analyze Mode**: Clicks "Analyze Current Plan" → **Visual animation shows each day being analyzed sequentially with pulsing ring effect** → AI evaluates nutritional balance showing daily averages (calories, protein, carbs, fat), meal variety score, cost per day, **and comprehensive sustainability metrics** (average sustainability score, CO₂ footprint, regional/organic/seasonal meal counts, transport distance, recyclable packaging percentage) → **Profile-aware insights include**: Critical allergen violation warnings if any meals contain excluded allergens (highlighted in red with organization name), budget target comparison showing cost per serving vs. profile target, dietary preference alignment check, **detailed sustainability performance rating with actionable recommendations based on score tiers (Excellent 80+: celebrates achievements with specific percentages and highlights alignment with Apetito sustainability commitments; Good 60-79: acknowledges baseline with specific improvement suggestions; Action Needed <60: provides concrete alternatives and emphasizes Apetito's high-sustainability options)** → Generates actionable insights with severity levels prioritizing profile compliance → Each insight includes specific action buttons filtered by profile (e.g., meal suggestions automatically exclude profile allergens) → User reviews AI-generated plan in Calendar view with color-coded days → Each meal shows AI reasoning for selection including profile considerations → **Shopping List tab displays consolidated meal summary** with unique meal count, total servings, and cost estimates compared to budget target → **User clicks "Place Order" button in Shopping List header** → System converts all planned meals into Product format with correct quantities → Adds each meal to cart as CartItem → Toast notification confirms successful addition ("24 meals added to cart! Total: €182.50") → User can proceed to Cart to finalize order
- **Success criteria**: Meal plans persist across sessions; **All meals in catalog updated to traditional English cuisine** (24 meals including Fish & Chips, Shepherd's Pie, Roast Chicken with Yorkshire Pudding, Bangers & Mash, Beef/Chicken Pies, Lancashire Hotpot, Full English Breakfast, Toad in the Hole, Jacket Potato, Ploughman's Lunch, Coronation Chicken, Beef Wellington, Mushy Peas, Roast Potatoes, Bread & Butter Pudding, Sticky Toffee Pudding, Apple Crumble, Scones, Eton Mess, Vegetable Wellington, Cauliflower Cheese); **Organization profile loads automatically in AI Assistant 100% of the time**; **Profile context visible in UI banner with color-coded allergen/dietary highlights**; **AI prompts explicitly include profile data** (organization name, type, allergen exclusions, dietary preferences, budget targets, special requirements); AI auto-fill generates balanced plans within 5 seconds **that comply with profile allergen exclusions 100% and feature English meals exclusively**; Custom requests successfully interpret 90%+ of natural language inputs **while maintaining profile compliance and English meal selection**; **Analysis animation shows each day being processed with visual feedback (400ms per day)**; Analysis completes within 2 seconds; Generated plans meet 2000-2200 kcal/day target with 60-80g protein; **AI excludes all meals containing profile allergen exclusions without manual specification**; **AI prioritizes meals matching profile dietary restrictions from English meal options**; Budget constraints honored from profile **with automatic cost-per-serving calculations against target**; Institutional parameters (people count) default to profile serving capacity; Natural language changes apply to correct days **while respecting profile constraints**; **AI explicitly references profile in reasoning**; **Allergen violation warnings appear prominently** if plan contains restricted allergens; **All meal suggestion functions filter by profile allergens**; Cost estimation accurate per serving **with comparison to profile budget target**; **Sustainability insights appear with three-tier system**; Multi-tab interface clearly separates generation, customization, and analysis workflows; **"Place Order" button visible in Shopping List tab header with ShoppingCart icon**; **Button click converts all meals (regardless of checked state) to Products and adds to cart with correct serving quantities**; **Toast notification confirms addition with meal count and total cost**; **Order placement completes within 1 second**; **Cart receives all planned meals as individual line items**
...
- **Search**: `MagnifyingGlass` for search input, `Sparkle` for AI semantic search toggle (filled when active), `X` for clear search
